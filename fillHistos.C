...
void fillHistos::fillBasic(basicHistos *h)
{
  ...
  if (gen_njt>=2 && _jp_ismc) { // Calculating GEN-LEVEL dijet mass
    ...
    // Find leading jets (residual JEC may change ordering)
    map<double, int> gen_ptorder;
    for (int i = 0; i != gen_njt; ++i) {
      double gen_idx = -gen_jtpt[i]; // note minus
      while (gen_ptorder.find(gen_idx) != gen_ptorder.end()) gen_idx += 1e-5*gen_jteta[i];
      assert(gen_ptorder.find(gen_idx)==gen_ptorder.end());
      gen_ptorder[gen_idx] = i;
    }
  ...
  if (gen_njt>=2 && _jp_ismc && njt>=2) { 
    ...
    _j1.SetPtEtaPhiE(jtpt[k0],jteta[k0],jtphi[k0],jte[k0]);
    _j2.SetPtEtaPhiE(jtpt[k1],jteta[k1],jtphi[k1],jte[k1]);
    
    double djmass = (_j1+_j2).M();
    double ymaxdj = max(fabs(jty[k0]),fabs(jty[k1]));
    bool goodmass = (jtpt[i0]>30. && jtpt[i1]>30.);
